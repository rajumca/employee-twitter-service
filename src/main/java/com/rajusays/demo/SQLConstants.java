...
public final class SQLConstants {
	...
	public static final String SELECT_FEED = "SELECT tweet.id, tweet.message, tweet.time, tweet.user_name, tweet_user.name FROM twitter.tweet as tweet, twitter.user as tweet_user WHERE  tweet_user.user_name=tweet.user_name AND (tweet.user_name in (select followee from twitter.followers where follower=:user_name) OR tweet.user_name=:user_name) order by time desc fetch first 100 rows only";
	public static final String SELECT_TWEET = "SELECT tweet.id, tweet.message, tweet.time, tweet.user_name, tweet_user.name FROM twitter.tweet as tweet, twitter.user as tweet_user WHERE  tweet_user.user_name=tweet.user_name AND tweet.user_name=:user_name order by time desc fetch first 100 rows only";
